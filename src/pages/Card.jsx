...
export default function Card(props) {
  ...
  async function SaveCard() {
    ...
    const result = await respond.json().then((data) => {
      return data;
    });
  ...
  const sendShareBtn = sendButton ? (
    <div
      className="preview-custom-btn send-btn"
      onClick={() => setSendButton(false)}
    >
      Send
    </div>
  ) : (
    <CardSharing
      username={username}
      /* contactName={contactName} */ url={cardLink}
      title={"A special Card for you!"}
      setSendButton={() => setSendButton(true)}
    />
  );
    ...
    <div id="preview-container">
      ...
      >
        <div className="preview-envelope">
          ...
          <div
            ref={previewEnvelopeLeftRef}
            id="preview-envelope-left"
            className="preview-envelope-left"
          >
            <img src={previewEnvelopeLeft} alt="envelope" />
          </div>
          <div
            ref={previewEnvelopeRightRef}
            id="preview-envelope-right"
            className="preview-envelope-right "
          >
          ...
          <div
            ref={previewFlipCardRef}
            id="preview-flip-card"
            className={`preview-flip-card ${
              isClicked ? "preview-translate" : "preview-reverse-translate"
            }`}
          >
            <div
              className={`preview-imgBox ${
                isClicked ? "preview-open-card" : "preview-close-card"
              }`}
            ...
            <div
              className="preview-flip-card-text"
              onClick={() => setIsClicked((state) => !state)}
            >
              {selectedText ? (
                ...
                <div className="preview-empty-text">
                  ...
                  <h3>
                    Please compose a message <br /> or choose a text template
                    from <br /> the Text Catalog.
                  </h3>
                  <h4>Greet Now</h4>
                </div>
              )}
      ...
      {isAuth ? (
        ...
        <div className="preview-footer">
          ...
          <div>
            <a>Please </a>
            <a
              style={{ cursor: "pointer", fontWeight: "bold", color: "tomato" }}
              onClick={() => {

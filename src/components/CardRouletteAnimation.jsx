...
const CardRoulette = (props) => {
  ...
  useEffect(() => {
    let urlPictures =
      "http://localhost:8080/media-catalog/getPictures?" +
      "&" +
      currEvent +
      "&";
    fetch(urlPictures).then((respond) =>
      respond.json().then((result) => {
        if (result.status == "success") {
          const pictures = result.data.pictures;
          const itemArr = [];
          pictures.map((pic) =>
            itemArr.push(`http://localhost:8080/greeting-pictures/${pic.name}`)
          );
          setItems(itemArr);
  ...
  function init(firstInit = true, groups = 1, duration = 1) {
    for (const door of doors) {
      ...
      for (let i = pool.length - 1; i >= 0; i--) {
        ...
        box.src = pool[i];
        box.onclick = () =>
          selectImg(box.src.substring(box.src.lastIndexOf("/") + 1));
  ...
  return (
    <section id="roulette-animation-container">
      ...
      <div
        id="card-roulette-btn"
        className="card-roulette-button glow-on-hover"
      >
        <Link to="#" id="spinner" className="roulette-btn" onClick={animate}>
          SPIN
        </Link>
      </div>
    </section>
